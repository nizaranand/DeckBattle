...
function importDeckedBuilder($userid, $deckfile,$deckname_input){
...
$deckname = str_replace(".txt","",$deckname_input);
...
if ($stmt = $mysqli->prepare("SELECT id FROM user_decks WHERE userid = ? AND deckname = ?")) { 
	...
	{ //deck exists  ...delete all related cards. Overwrite standard (later .. make versions
		 
		$stmt = $mysqli->prepare("DELETE FROM user_decks_cards WHERE deckid = ?"); 
		$stmt->bind_param('s', $deckid); 
		$stmt->execute(); 
		 $info = "DeckedBuilder file uploaded and processed. Existing deck overriden.";
	}
}
