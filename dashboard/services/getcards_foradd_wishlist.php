 ...
 */
$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS `" . str_replace(" , ", " ", implode("`, `", $aColumns)) . "`, (SELECT amount_normal FROM user_wishlist col WHERE col.cardid = c.Ncardid AND userid=". $userid ." LIMIT 1) as n, (SELECT amount_foil FROM user_wishlist col WHERE  col.cardid = c.Ncardid AND userid=". $userid ." LIMIT 1) as f, (SELECT COUNT(id) FROM user_favoritecards fav WHERE  fav.cardid = c.Ncardid AND userid=". $userid ." LIMIT 1) as fav 
		FROM   $sTable
		$sWhere
		$sOrder
		$sLimit
