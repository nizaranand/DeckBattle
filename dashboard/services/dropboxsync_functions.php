...
function addFilesToDropbox($db, $mysqli) {
	if ($stmt = $mysqli -> prepare("SELECT id, deckname, updated,dropboxPath,extension,deckimage,isFavorite,color FROM user_decks WHERE userid = ? AND markfordropbox = 1")) {
		...
		$stmt -> execute();
		$stmt -> store_result();
		$stmt -> bind_result($deckid, $db_deckname, $updated, $dropboxPath, $extension, $deckimage, $fav, $color);
			...
			if (!checkIfDeckInDropbox($content, $db_deckname, $db)) {
				...
				if ($stmt1 = $mysqli -> prepare("SELECT cardid, amount_normal, amount_foil, location FROM user_decks_cards WHERE deckid = ? ")) {
					...
					while ($stmt1 -> fetch()) {
						...
						if ($extension == ".txt") {// simple ; separated including foil but can be discarded
