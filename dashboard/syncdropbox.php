...
$storage->connect(HOST, DATABASE,USER,PASSWORD, 3306);
...
if ($stmt = $mysqli->prepare("SELECT id, deckname, updated,dropboxPath, extension FROM user_decks WHERE userid = ? AND markfordropbox = 1")) { 
	...
	while ($stmt->fetch()) {
		...
		if (!checkIfDeckInDropbox($content,$db_deckname,$db)) {
			...
			function getFilesFromDropbox($content,$mysqli,$db){
			$max = sizeof($content);
			if ($max > 0) {
			for ($i=0; $i<$max; $i++) {
      ...
      <div class="widget grid6">
        <div class="whead">
          <h6>Synchronisation Listing |
            <?php
			if ($accountInfo["body"] -> email != "")
				echo "Dropbox account: " . $accountInfo["body"] -> email . "";
 ?>
          </h6>
          <div class="clear"></div>
        </div>
        <ul class="updates">
          ...
          <?php
		getFilesFromDropbox($metaData["body"] -> contents, $mysqli, $dropbox);
					?>
        </ul>
      </div>
      <div class="widget grid6">
