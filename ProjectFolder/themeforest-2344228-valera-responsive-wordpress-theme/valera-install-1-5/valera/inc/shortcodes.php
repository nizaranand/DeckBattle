...
function theme_blog( $atts, $content = null)
{
	...
	$output .= theme_blog_loop($show_posts);
	$output .= '</div>'."\n";
...
function theme_blog_loop($show_posts)
{
	$args = array( 
				'post_type' => 'post',
	...
	$output .= '<div class="row">'."\n";
		$output .= '<div class="blogslider span6 clearfix">'."\n";
	...
	while ($query->have_posts()) { $query->the_post();
		...
		$content = get_the_content('<br><a class="btn btn-small" style=" margin-top:15px;" href="'. get_permalink($post->ID) . '">Read More</a>');
		$author = get_the_author_meta('nickname');

		$output .= '<li>'."\n";
			$output .= '<div class="blog_item">'."\n";
				$output .= '<div class="blog_head">'."\n";
					$output .= '<h3><a href="'.get_permalink().'"> '.$title.'</a></h3>'."\n";
								$output .= '<span><strong><i class="icon-user icon-white"></i> Author:</strong> '.$author.'</span>'."\n";
                                $output .= '<span><strong><i class="icon-comment icon-white"></i> Comments:</strong> <a href="'.get_permalink().'#comments">'.get_comments_number('0','1','%').'</a></span>'."\n";
                                $output .= '<span><strong><i class="icon-user icon-white"></i> Date:</strong> '.get_the_time('d M Y').'</span>'."\n";
				...
				$output .= '<div style="padding:20px;">'.do_shortcode($content).'</div>'."\n";
			$output .= '</div>'."\n";
	$output .= '</li>'."\n";
		...
		$descr = get_post_meta($post_id, 'port-descr', 1);
			...
			$output .= '<div class="span3 block" style="margin-bottom:20px !important;">'."\n";
				$output .= '<div class="view view-first">'."\n";
					...
					$output .= '</div>'."\n";
				$output .= '</div>'."\n";
				$output .= '<h5><i class="'.$icon.'"></i> <a href="'.get_permalink().'">'.$title.'</a></h5>'."\n";
...
{	
	...
	$output = '<div class="view view-first '.$size1.' '.$margin1.' '.$last1.'" style="margin-bottom:20px;">'."\n";
	$output .=	'<a href="'.$image.'" rel="prettyPhoto"><img src="'.$image.'" alt="" /></a>'."\n";
		$output .=	'<div class="mask">'."\n";
			$output .=	'<a href="'.$image.'" rel="prettyPhoto" class="info" style=" margin-left:-25px !important;"><img src="'.$default_url.'/assets/img/plus.png" alt=""/></a>'."\n";
		$output .=	'</div>'."\n";
